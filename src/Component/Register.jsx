...
export default function Register() {
  ...
  let validationSchema = Yup.object({
    name: Yup.string().max(10, 'max length is 10').min(2, 'at least 2 char').required('name is required'),
    email: Yup.string().email().required('email is required'),
    password: Yup.string().required('password is required').matches(/^[A-Z][a-z0-9]{5,10}$/),
    rePassword: Yup.string().oneOf([Yup.ref('password')], 'password did not match').required('repassword is required'),
    phone: Yup.string().matches(/^01[0-25][0-9]{8}$/i).required('phone is required'),
  })
 ...
 if (msg)
  ...
  return (
    <div>
      <h2 className='p-5 my-2 text-green-900 text-2xl font-extrabold'>Register Now:</h2>

      {msg ? <div className="p-4 w-1/2 mx-auto mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
      ...
      <form className="max-w-md mx-auto" onSubmit={formik.handleSubmit}>
        ...
        <div className="relative z-0 w-full mb-5 group">
          ...
          <label htmlFor="floating_phone" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-green-600 peer-focus:dark:text-green-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Phone number (+01029000025)</label>
        </div>
        {/* phone alert */}
